...
class BooksModel:
    ...
    def create_book(self, user_id: int, title: str, description: str, file_url: str, 
        ...
        """
        Create a new book record with transaction support.
        
        Args:
            user_id: ID of the user creating the book
            title: Title of the book
            description: Description of the book
            file_url: URL to the book file
            audio_url: URL to the audio version (optional)
            is_public: Whether the book is public
            is_approved: Whether the book is approved
            uploaded_by_role: Role of the uploader
        ...
        try:
            with conn.cursor() as cur:
                ...
                if category_ids:
                    ...
                    # Insert only valid category associations
                    if valid_ids:
    ...
    def delete_book(self, book_id: int) -> bool:
        """
        Delete a book and all its related records with transaction support.
            ...
            with conn.cursor() as cur:
                ...
                for table in related_tables:
                    try:
                        ...
                        logger.warning(f"Error deleting from {table} for book {book_id}: {e}")
                        continue
    ...
    async def get_top_books(limit=5):
        try:
            async with pool.acquire() as conn:
                async with conn.transaction():
                    query = """
                        SELECT b.id, b.title, COUNT(br.id) AS read_count
                        FROM books b
    ...
    def get_category_distribution(self) -> List[Dict]:
        """
        Get the distribution of books across categories.
        ...
        try:
            ...
            logger.error(f"Failed to fetch category distribution: {e}")
            return []
        finally:
            if 'cur' in locals():
                cur.close()
